...
def compute_intersect(ps1, ps2):
    pa = []
    pb = []
    for p in ps1:
        pa.append(Point(p[0], p[1]))
    for p in ps2:
        pb.append(Point(p[0], p[1]))
    
    return NPIA(pa, pb)


# s = np.array([[0.0, 0.0], [1.0, 0.0], [1.0,1.0], [0.0, 1.0]])
# # t = np.array([[0, 0], [1.0, 1.0], [1.0,0], [0.0, 1.0]])
# t = np.array([[1,1], [0,1.0], [0,0], [1,0]])
# print(compute_iou(s, t))
# compute_intersect(s, t)