...
class BasicPattern(object):
    ...
    def name_from_path(pattern_file):
        #basename():get the last file name; splitext():split the file name and extension name
        name = os.path.splitext(os.path.basename(pattern_file))[0]#just the pattern spec json file
        if name in standard_filenames:  # use name of directory instead
            #dirname():remove the file name and keep the path
    ...
    def _edge_as_vector(self, vertices, edge_dict):
        ...
        curvature1 = np.array(edge_dict['curvature'][0]) if 'curvature' in edge_dict else [0, 0]
        curvature2 = np.array(edge_dict['curvature'][1]) if 'curvature' in edge_dict else [0, 0]


        return np.concatenate([edge_vector, curvature1, curvature2])
    ...
    def _normalize_template(self):
        ...
        if ('normalize_panel_translation' in self.properties 
            ...
            for panel in self.pattern['panels']:
                ...
                #update absolute curvature
                for i, edge in enumerate(self.pattern['panels'][panel]['edges']):
                    if 'curvature' in edge:
                        curvature = np.array(self.pattern['panels'][panel]['edges'][i]['curvature'][0]) - offset
                        self.pattern['panels'][panel]['edges'][i]['curvature'][0] = curvature.tolist()
                        curvature = np.array(self.pattern['panels'][panel]['edges'][i]['curvature'][1]) - offset
                        self.pattern['panels'][panel]['edges'][i]['curvature'][1] = curvature.tolist()
    ...
    def _normalize_panel_scaling(self, panel_name, units_in_meter):
        ...
        #control point
        for i, edge in enumerate(self.pattern['panels'][panel_name]['edges']):
            if 'curvature' in edge:
                vertices = np.array(self.pattern['panels'][panel_name]['edges'][i]['curvature'])
                vertices = scaling * vertices
                self.pattern['panels'][panel_name]['edges'][i]['curvature'] = vertices.tolist()
        ...
        if np.cross(first_edge, last_edge) > 0:  # should be negative -- counterclockwise
            ...
            for edge_id in range(len(rotated_edges)):
                if 'curvature' in panel['edges'][edge_id]:
                    ...
                    #   8*********** for our pattern 
                    curvature = np.array(panel['edges'][edge_id]['curvature'])
                    curvature[:, 0] *= -1
                    panel['edges'][edge_id]['curvature'] = curvature.tolist()
    ...
    def _is_panel_self_intersecting(self, panel_name):
        ...
        for edge in panel['edges']:
            ...
            if 'curvature' in edge:
                ...
                #--------------edit fro our pattern--------------
                curv_abs = edge['curvature']
                # view curvy edge as 3 segments
                # NOTE this aproximation might lead to False positives in intersection tests
                edge_list.append([edge_coords[0], curv_abs[0]])
                edge_list.append([curv_abs[0], curv_abs[1]])
                edge_list.append([curv_abs[1], edge_coords[1]])
            else:
