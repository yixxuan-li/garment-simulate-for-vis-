...
class MayaGarment(core.ParametrizedPattern):
    ...
    def load(self, obstacles=[], shader_group=None, config={}, parent_group=None):
        ...
        self.add_colliders(obstacles)   #in this code, add the smpl body model as the colliders
    ...
    #---------------- save_mesh_all() ------------------
    def save_mesh_all(self, folder='', tag='sim'):
        """
            Saves cloth and smpl model as obj file and its per vertex segmentation to a given folder or 
            to the folder with the pattern if not given.
        """
        if not self.loaded_to_maya:
            print('MayaGarmentWarning::Pattern is not yet loaded. Nothing saved')
        ...
        if folder:
            filepath = folder
            self._save_to_path_all(filepath, tag)  
        else:
            filepath = self.path
            self._save_to_path_all(filepath, self.name + '_' + tag)        
    #---------------- save_mesh_clo() ------------------
    def save_mesh_clo(self, folder='', tag='sim'):
        """
            Saves cloth and smpl model as obj file and its per vertex segmentation to a given folder or 
            to the folder with the pattern if not given.
        """
        ...
        if folder:
            ...
            filepath = self.path
        self._save_to_path_clo(filepath, tag)      
    ...
    def uv_layout(self):
        # qlcloth = self.get_qlcloth_geomentry()
        # uv_shell_count = cmds.polyEvaluate(self.get_qlcloth_geomentry(), uvShell = True)
        # uv_shell = cmds.polyEvaluate(self.get_qlcloth_geomentry(), uis = uv_shell_count -1)#select front uvShell
        # cmds.select(uv_shell)
        # cmds.polyEditUV( relative= True, uValue= 0.5, vValue=0.0 )
        # cmds.polyLayoutUV('qlCloth3Out' ,  l=1, fr=True, se=0, sc=1, lm=1, percentageSpace = 5, caching = False )
        cmds.polyLayoutUV(self.get_qlcloth_geomentry() + '.f[:]' ,  l=1, fr=True, se=0, sc=1, lm=1, percentageSpace = 5, caching = False )
        # cmds.polyLayoutUV('qlCloth2Out.f[:]' ,  l=2, fr=True, se=2, sc=1 )
    # ------ Simulation ------
    ...
    """
    #----------------------my get_qlcloth_geomentry()-----------------
    def get_qlcloth_geomentry(self):
        """
            Find the all Qualoth cloth geometry object belonging to current pattern
        """
        if not self.loaded_to_maya:
            raise RuntimeError('MayaGarmentError::Pattern is not yet loaded.')
    ...
    #---------------------my get_qlcloth_props_obj()--------------------------
    def get_qlcloth_props_obj(self):
        """
            Find the all qlCloth object belonging to current pattern
        """
        if not self.loaded_to_maya:
            raise RuntimeError('MayaGarmentError::Pattern is not yet loaded.')
        ...
        if 'qlCloth' not in self.MayaObjects:
            ...
            cloths = [obj for obj in children 
                      if 'qlCloth' in obj and 'Out' not in obj and 'Shape' in obj]
            self.MayaObjects['qlCloth'] = cloths[0] #get all the cloth
    ...
    def get_qlcloth_geom_dag(self):
        ...
        if 'shapeDAG' not in self.MayaObjects:
            print("-----------------------------")
            print(utils.get_dag(self.get_qlcloth_geomentry()))
    ...
    def _setSimProps(self, config={}):
        ...
        """
        qw.setFabricProps(
            self.get_qlcloth_props_obj()[0], 
            self.config['material']
        )
        qw.setFabricProps(
            self.get_qlcloth_props_obj()[1], 
            self.config['material']
        )     
        """
        ...
        if 'collision_thickness' in self.config:
            ...
            """
            cmds.setAttr(self.get_qlcloth_props_obj()[0] + '.overrideThickness', 1)
            cmds.setAttr(self.get_qlcloth_props_obj()[0] + '.thickness', self.config['collision_thickness'])
            cmds.setAttr(self.get_qlcloth_props_obj()[1] + '.overrideThickness', 1)
            cmds.setAttr(self.get_qlcloth_props_obj()[1] + '.thickness', self.config['collision_thickness'])            
            """
    ...
    def _edge_as_3d_tuple_list(self, edge, vertices):
        ...
        if 'curvature' in edge:
            points = np.r_[
                [points[0]], [edge['curvature'][0]], [edge['curvature'][1]], [points[1]]
            ]
    ...
    def _save_to_path(self, path, filename):
        ...
        # utils.my_save_mesh(self.get_qlcloth_geomentry(), self.obstacles,  filepath)
        ...
        """ 
    #--------------------- _save_to_path_all() --------------------------        
    def _save_to_path_all(self, path, filename):
        """Save current state of cloth object to given path with given filename"""
        
        # geometry
        filepath = os.path.join(path, filename + '.obj')
        utils.my_save_mesh_all(self.get_qlcloth_geomentry(), self.obstacles,  filepath)
        ...
        """ 
    #--------------------- _save_to_path_clo() --------------------------        
    def _save_to_path_clo(self, path, filename):
        """Save current state of cloth object to given path with given filename"""
        
        # geometry
        filepath = os.path.join(path, filename + '.obj')
        utils.my_save_mesh_clo(self.get_qlcloth_geomentry(), self.obstacles,  filepath)
...
class Scene(object):
    ...
    """
    def __init__(self, body_obj, props, scenes_path='', clean_on_die=False, stage = None):
        """
        ...
        self._load_body(body_obj, stage = stage)
        ...
        if 'scene' in self.config:
            ...
            self._simple_scene_setup(stage = stage)
    ...
    def _init_arnold(self):
        ...
        cmds.setAttr('defaultArnoldRenderOptions.renderDevice',0)  # turn on GPPU rendering
    ...
    def render(self, save_to, name='last'):
        ...
        dirs = os.path.join(save_to, 'rendering')
        if not os.path.exists(dirs):
            os.makedirs(dirs)


        #set the path to save 
        pm.workspace(fileRule=['images', dirs])     
        ...
        # render all the cameras
        start_time = time.time()
        camera_tag = ['front', 'back']
        for id, camera in enumerate(self.cameras):
            print('Rendering from camera {}'.format(camera_tag[id]))
            cmds.setAttr("defaultRenderGlobals.imageFilePrefix", camera_tag[id], type = "string")
            cmds.hwRender(cam = camera, h = im_size[1], w = im_size[0])

            #get camera paras
            paras = {}
            paras['translation'] = cmds.getAttr('%s.translate'%camera)
            paras['focal_Length'] = cmds.getAttr('%s.focalLength'%camera)
            paras['rotation'] = cmds.getAttr('%s.rotate'%camera)
            out = open((dirs + "\\" + camera_tag[id] + "_paras.json"), 'w')
            out.write(json.dumps(paras)) 
            out.close()
    ...
    def _simple_scene_setup(self, stage = None):
        ...
        if stage == 1:
            self.scene['body_shader'], self.scene['body_SG'] = self._body_texture( target =self.body)
        self.scene['cloth_shader'], self.scene['cloth_SG'] = self._new_texture( target = self.body)
        ...
        self.scene['floor_shader'], self.scene['floor_SG'] = self._new_lambert(colors['floor_color'])
        ...
        cmds.file(
            'C:/Users/12790/Desktop/test1.',
            type='OBJExport',  
            exportSelectedStrict=True,  # export selected -- only explicitely selected
            options='groups=0;ptgroups=0;materials=1;smoothing=0;normals=1',  # very simple obj
        ...
        self._set_aiskydomelighttexture()
        cameras = self._add_simple_camera()
        # Put camera
        self.cameras = [cameras[0],cameras[1]]
    ...
    def _add_simple_camera(self, rotation=[-23.2, 16, 0]):
        ...
        rotation[0] = random.uniform(-25, 25)
        rotation[1] = random.uniform(-20, 20)
        ...
        cmds.viewPlace(camera, perspective = True)

        ##************************ 
        # # set the back camera
        camera2 = cmds.camera(aspectRatio=self.config['resolution'][0] / self.config['resolution'][1])[0]
        rotation[0] = random.uniform(-25, 25)
        rotation[1] = random.uniform(-20, 20)
        cmds.setAttr(camera2 + '.rotate', rotation[0], rotation[1], rotation[2], type='double3')
        # to view the target body
        fitFactor = self.config['resolution'][1] / self.config['resolution'][0]
        cmds.viewFit(camera2, self.body, f=fitFactor)
        x = cmds.getAttr('%s.translateX'%camera2)
        z = cmds.getAttr('%s.translateZ'%camera2)
        y = cmds.getAttr('%s.translateY'%camera2)
        cmds.viewPlace('%s'%camera2, eyePoint =(-x, y, -z) ,perspective = True)
    ...
    def _add_floor(self, target):
        ...
        return floor[0]
    def _set_aiskydomelighttexture(self):
        if 'light_path' in self.config:
            file = self.config['light_path']
            filenode = cmds.shadingNode('file', at = True)
            tex = cmds.shadingNode('place2dTexture', asUtility = True)
            cmds.setAttr('%s.fileTextureName'%filenode, file, type = "string")
            cmds.connectAttr('%s.outColor'%filenode, '%s.color'%self.scene['light'][1])
            cmds.connectAttr(tex+'.outUV', filenode+'.uvCoord')
            cmds.connectAttr(tex + '.outUvFilterSize', filenode + '.uvFilterSize')
    ...
    def _body_texture(self, target = None):
        """created a new shader node with given texture"""
        if 'bodytexture_path' in self.config:
            file = self.config['bodytexture_path']
        else:
            file = 'C:/Users/12790/Desktop/texture/body/grey_female_0019.jpg'
        ...
        shader_group = cmds.sets(renderable=True, noSurfaceShader=True, empty=True, name = 'shader')
        cmds.connectAttr('%s.outColor'%shader, '%s.surfaceShader'%shader_group)
        cmds.connectAttr('%s.outColor'%filenode, '%s.color'%shader)
        cmds.connectAttr(tex+'.outUV', filenode+'.uvCoord')
        cmds.connectAttr(tex + '.outUvFilterSize', filenode + '.uvFilterSize')
        # #edit the uv repeat
        # cmds.setAttr(tex + '.repeatU', 5)
    ...
    def _new_texture(self, target = None):
        """created a new shader node with given texture"""
        if 'texture_path' in self.config:
            file = self.config['texture_path']
        else:
            file = 'C:/Users/12790/Desktop/texture/pic/076.jpg'
