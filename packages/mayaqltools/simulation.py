...
# ----------- High-level requests --------------
def single_file_sim(resources, props, tag = None , caching=False, render =False, stage = None):
    """
    ...
    try:
        ...
        template_simulation(props['pattern_path'], scene, props['sim'], folder = props['folder'], tag = tag , caching=caching, render =render, stage = stage)


        scene.self_clean = True
        scene.__del__()
        # Fin
        print('\nFinished experiment')
        try:
...
def batch_sim(resources, data_path, dataset_props, num_samples=None, caching=False, force_restart=False):
    ...
    #the path to generated data's template spec json file 
    pattern_specs = _get_pattern_files(data_path, dataset_props)

    #the path to generated data's data properties
    ...
    for pattern_spec in pattern_specs:
        ...
        pattern_spec_norm = os.path.normpath(pattern_spec)#to normorlize the path 
...
def template_simulation(spec, scene, sim_props, folder = None, tag = None, delete_on_clean=False, caching=False, render = False, save_maya_scene=False, stage = None):
    """
        Simulate given template within given scene & save log files
    """
    print('\nGarment load')
    garment = mymaya.MayaGarment(spec)#load the spec.json file
    ...
    try:
        ...
        #garment UV layout and texture load 
        garment.uv_layout()
        ...
        if tag == '_up':
            garment.save_mesh_all(folder = folder, tag = 'model')
        else:
            garment.save_mesh_all(tag='sim_with_smpl')

        garment.save_mesh_clo(folder= folder, tag = tag)
        if render:
            # scene.render(garment.path, garment.name)
            scene.render(folder, garment.name)
        if save_maya_scene:
