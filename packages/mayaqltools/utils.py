...
# ----- Working with files -----
def load_file(filepath, name='object', stage = 1):
    ...
    if stage == 1:
        ...
        #body with textuer
        body = cmds.file(filepath, i=True, rnn=True)
        obj = body[-2]
...
#--------------- my_save_mesh_all()--------------------
def my_save_mesh_all(target, target2, to_file):
    """Save given object and smpl model to file as a mesh"""

    # Make sure to only select requested mesh
    cmds.select(clear=True)
    cmds.select(target, target2)
    ...
    cmds.select(clear=True)
#--------------- my_save_mesh_clo()--------------------
def my_save_mesh_clo(target, target2, to_file):
    """Save given object and smpl model to file as a mesh"""
